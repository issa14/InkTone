...
> **Mise à jour 2026-07-20** : cas réel signalé — import de 500 EPUB, plus de 15 minutes et toujours en cours. Deux causes supplémentaires identifiées en creusant ce cas précis, en plus de celle documentée ci-dessous. Voir le détail complet dans `PLAN_ACTION_TOP_TIER_CLAUDECODE.md`, Phase 2 (diagnostic à trois causes : mode journal SQLite `TRUNCATE` au lieu de `WAL`, absence totale de concurrence dans `LibraryViewModel.importBooks()` pour le traitement du lot, et la cause ci-dessous démultipliée par le nombre de livres).
