...
### 7.0/7.1.1 — Révision après implémentation réelle : trois prémisses fausses, trouvées en écrivant le code, pas en le lisant

**1. Le Reader n'affichait pas de chapitre du tout.** La prémisse de 7.0/7.1 (« ReaderScreen rend le chapitre dans un `Column` standard ») supposait un rendu continu du texte. En réalité `ReaderScreen` (héritage Phases 3/4) n'affichait qu'**une seule `Sentence` à la fois** — squelette pour tester le pipeline TTS, pas la « lecture visuelle » que le Blueprint exige (ligne 89) en complément du mode audio. Il n'y avait donc rien sur quoi sélectionner du texte multi-phrases. Résolu en construisant d'abord ce rendu continu (toutes les `Sentence` du chapitre, `FlowRow` défilable via `verticalScroll`, jamais `LazyColumn` — cohérent avec l'avertissement que cette tâche voulait vérifier).

**2. L'API `SelectionContainer` contrôlée n'existe pas publiquement.** `SelectionContainer(state = rememberSelectionState())` (pseudo-code d'origine de cette tâche) ne correspond à aucune API réelle. La vraie signature à deux paramètres,
`SelectionContainer(modifier, selection: Selection, onSelectionChange: (Selection) -> Unit, content)`,
existe bien dans `androidx.compose.foundation:foundation:1.7.2` (BOM 2024.09.02) — mais elle est **`internal`**, tout comme la classe `Selection` elle-même. Confirmé par le compilateur Kotlin en essayant de compiler cette tâche (`Cannot access 'data class Selection : Any': it is internal in file`), pas supposé d'après une documentation générique. Aucune API publique ne donne accès aux offsets d'une sélection de texte native à ce niveau de Compose Foundation.

**Décision actée (validée par Issa)** : sélection **par phrase**, pas par caractère arbitraire. Appui long sur une phrase démarre une sélection, appui simple sur une autre phrase l'étend — l'index de `Sentence` est connu par construction (chaque phrase est son propre composable dans le `FlowRow`), donc `AnnotationSelectionHandler` n'a plus besoin de chercher quelle `Sentence` contient un offset : il reçoit directement les index de début/fin. UX plus rustique qu'une sélection native à poignées, mais 100% API publique stable (`Modifier.combinedClickable`, aucune dépendance à `SelectionContainer`). Alternatives écartées : reconstruire une sélection libre par pixel via `TextLayoutResult.getOffsetForPosition` (un vrai morceau d'UI à maintenir, équivalent à réimplémenter une partie de `SelectionContainer`) ; intercepter le Copier d'un `SelectionContainer` simple via un `TextToolbar` personnalisé puis retrouver la position par `indexOf` (fragile si le texte sélectionné apparaît plusieurs fois dans le chapitre).

**3. Le critère de validation par redémarrage de l'app ne peut pas être vérifié tel quel avec le squelette actuel — pas un bug de cette tâche.** En testant sur device, l'annotation créée disparaissait après avoir fermé/rouvert l'app. Investigation par inspection directe de la base SQLite (fichier `.db` + `.db-wal`, la base tourne en WAL — copier seulement `.db` sans le `.db-wal` donne un faux négatif, piège rencontré en vérifiant) : l'annotation est bien écrite et persistée (`SELECT * FROM annotations` la montre juste après création, y compris après checkpoint WAL). La disparition vient de `ReaderViewModel.bootstrapAndOpenFixture()` (scaffolding de marche à blanc, Phase 3), appelé à **chaque lancement** de `MainActivity` avec le même `publicationId` fixe — `PublicationDao.insert` utilise `OnConflictStrategy.REPLACE`, qui fait un DELETE+INSERT réel sur conflit de clé primaire, et `AnnotationEntity` a une contrainte `onDelete = CASCADE` vers `publicationId` : chaque relance supprime donc silencieusement toutes les annotations de la fixture. Confirmé en comparant le compte de lignes de `annotations` avant/après relance (1 → 0) via `adb exec-out run-as`. **Non corrigé ici** : `bootstrapAndOpenFixture` est un scaffolding déjà documenté comme temporaire (« à retirer quand feature/library fournira un import réel ») — le corriger maintenant serait hors du périmètre de cette tâche et risquerait d'affecter les tests de reprise K3 existants (`ReadingResumeTest`) qui dépendent peut-être de ce comportement exact. Le chemin d'écriture/lecture de l'annotation est prouvé correct indépendamment de ce défaut préexistant.
