...
**État d'implémentation (lot 11) :** Synchronisation n'est plus masquée — réactivée en **b5** de la liste principale du drawer (entre Marque-pages et Statistiques), pas au pied de drawer comme une première implémentation l'avait placée à tort (retour Issa, vérification device, corrigé le jour même). **Toutes les destinations conçues sont désormais affichées** — seule Catalogues OPDS (b4) reste masquée, réintégrée en v1.x et planifiée par le Lot 13 (`ADR-023`, `docs/execution/LOT_13_CATALOGUES_OPDS.md`) ; elle n'a jamais été conçue dans cette session, ce n'est pas un oubli.

**Google Drive en V1, WebDAV différé puis implémenté (décision actée, tâche 11.2 puis Lot 19)** : `drive.appdata` est une portée sensible mais non restreinte — 100 comptes de test disponibles sans attente en mode Test, aucun audit de sécurité tiers requis pour démarrer. L'exclusivité mutuelle actée dans la cible (un seul fournisseur cloud actif) impose de toute façon une frontière propre entre fournisseurs ; WebDAV est la seconde implémentation derrière la même interface `SyncProvider`, livrée par le Lot 19 (`WebDavSyncProvider` + `SyncProviderRouter`, carte Configuration fonctionnelle : champs URL/Identifiant/Mot de passe, test de connexion, badge Connecté, Déconnecter, grisage réciproque avec Drive). **Écart déclaré (Lot 19) :** le mot de passe WebDAV n'est jamais ré-affiché une fois connecté (secret, même discipline que le jeton OAuth) — la carte active montre l'hôte + « Connecté », pas les champs d'édition.
